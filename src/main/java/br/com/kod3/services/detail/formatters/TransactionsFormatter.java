...
public class TransactionsFormatter implements Formatter {
    ...
    private String buildResponseList(BigDecimal totalSpent, BigDecimal totalSaved,
                                     Map<LocalDate, List<Transaction>> dailyTransactions,
                                     Map<YearMonth, Map<Category, BigDecimal>> monthlySpending,
                                     Integer allTimeTotalSpent, Integer allTimeTotalSaved) {

        StringBuilder response = new StringBuilder();
        response.append("📄 *Resumo de Transações*\n");

        dailyTransactions.forEach((date, txList) -> {
            response.append("\n📅 *").append(date.format(DATE_FORMATTER_PT_BR)).append("*\n");

            // Agrupa primeiro por tipo (despesa/receita), depois por categoria
            Map<TransactionType, Map<Category, List<Transaction>>> byType = txList.stream()
                    .sorted(Comparator.comparing(Transaction::getCategory))
                    .collect(Collectors.groupingBy(Transaction::getType,
                            LinkedHashMap::new,
                            Collectors.groupingBy(Transaction::getCategory, LinkedHashMap::new, Collectors.toList())));

            byType.forEach((type, categoryMap) -> {
                ...
                categoryMap.forEach((category, transactions) -> {
                    ...
                    for (Transaction tx : transactions) {
                        String symbol = "";
                        if (firstOfType) {
                            symbol = type == TransactionType.EXPENSE ? "\uD83D\uDCC9" : "\uD83D\uDCC8";
                            firstOfType = false;
                        }
        ...
        response.append(String.format("🧾 Saldo líquido: *%s%.2f*\n", CURRENCY_SYMBOL, totalSaved.subtract(totalSpent)));
        ...
        if (!monthlySpending.isEmpty()) {
            ...
            monthlySpending.forEach((month, categories) -> {
                ...
                response.append(String.format("   📊 Total do mês: *%s%.2f*\n", CURRENCY_SYMBOL, monthTotal));
            });
        }

        // ---
        // Cálculo do total de receitas bloqueadas do período
        BigDecimal allTimeBlockedIncome = dailyTransactions.values().stream()
                .flatMap(List::stream)

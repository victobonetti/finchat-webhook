    ...
    public static ConvertedDto parse(WebhookBodyDto dto){
        ...
        if (type.equals(MessageType.listResponseMessage)){
            ...
            try {
                ...
                var transactionType = dto.data().message().listResponseMessage().contextInfo().quotedMessage().listMessage().title()
                        .replace("Registrar ", "").equalsIgnoreCase("gasto") ?
                        TransactionType.EXPENSE :
                        TransactionType.INCOME;

                var transactionDto = TransactionPayloadDto.builder()
                        .business(q[0])
